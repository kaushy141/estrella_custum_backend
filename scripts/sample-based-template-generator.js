...
/**
 * Sample-Based Excel Template Generator using ExcelJS
 * Creates Excel files based on the exact structure of the sample invoice
 */
class SampleBasedTemplateGenerator {
    constructor() {
        this.workbook = null;
        this.worksheet = null;
    ...
    /**
     * Create workbook based on sample structure using ExcelJS
     */
     ...
     * Maps OpenAI JSON structure to Excel cells
     */
    getTranslatedValue(row, col, originalValue, jsonData, options) {
        ...
        // Map based on cell position and content using OpenAI JSON structure
        switch (cellAddress) {
            // Title
            case 'A1':
                return 'FAKTURA';
    ...
    /**
     * Get items data for specific rows using OpenAI JSON structure
     */
    getItemsData(row, col, jsonData, options) {
        ...
        const useConvertedValues = options.currencyConversion && options.currencyConversion.exchangeRate !== 1;
        ...
        switch (cellAddress) {
            ...
            case 'A35':
                ...
                if (typeof item.description === 'object' && item.description[targetLanguage]) {
                    return item.description[targetLanguage];
            ...
            case 'I35':
                ...
                if (typeof item.rate === 'object' && item.rate.converted_value !== undefined) {
                    return useConvertedValues ? item.rate.converted_value : item.rate.value;
            ...
            case 'J35':
                ...
                if (typeof item.amount === 'object' && item.amount.converted_value !== undefined) {
                    return useConvertedValues ? item.amount.converted_value : item.amount.value;
    ...
    setCellWithExcelJSFormat(row, col, value, templateCell) {
        ...
        if (templateCell.isTotal) {
            cellStyle.font.bold = true;
            cellStyle.alignment.horizontal = 'right';
        }

        if (templateCell.isData && typeof value === 'number') {
            cellStyle.alignment.horizontal = 'right';
            cell.numFmt = '#,##0.00';
        }
    ...
    /**
     * Apply merged cells from sample using ExcelJS
     */
    applyMergedCells() {
    ...
    /**
     * Apply formulas from sample using ExcelJS
     */
    ...
    /**
     * Save the workbook using ExcelJS
 ...
 * Test function
 */
async function testSampleBasedTemplate() {
    try {
        console.log('🧪 Testing ExcelJS Sample-Based Template Generator...');

        // Load template structure
        const generator = new SampleBasedTemplateGenerator();
        ...
        // Save workbook
        const outputPath = path.join(__dirname, 'test_output', 'exceljs-sample-based-invoice.xlsx');
        await fs.mkdir(path.dirname(outputPath), { recursive: true });
        await generator.saveWorkbook(outputPath);

        console.log('\n✅ ExcelJS sample-based template test completed successfully!');
...
module.exports = SampleBasedTemplateGenerator;